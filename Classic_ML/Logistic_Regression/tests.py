...
def test_function_negative_inf_values():
    ...
    X = np.array([[1, 10**5], [1, -(10**5)], [1, 10**5]])
    y = np.array([1, -1, 1])
    w = np.array([1, 100])
...
def test_function_positive_inf_values():
    loss_function = BinaryLogisticLoss(l2_coef=0.0)
    X = np.array([[1, 10**2], [1, -(10**2)], [1, 10**2]])
    y = np.array([-1, 1, -1])
    w = np.array([1, 100])
...
def test_function_sparse():
    loss_function = BinaryLogisticLoss(l2_coef=1.0)
    X = csr_matrix(np.array([[1, 1, 2], [1, 3, 4], [1, -5, 6]]))
    y = np.array([-1, 1, 1])
    w = np.array([1, 2, 3])
...
def test_full_gd(step_alpha, step_beta, answer):
    ...
    npt.assert_almost_equal(
        lm.loss_function.func(X, y, lm.get_weights()), answer, decimal=5
    )
