...
class SVD:
    ...
    def fit_sgd(self):
        ...
        for idx, row in self.sample.iterrows():
            ...
            err = r - (
                self.global_mean + self.bu[u] + self.bi[i] + self.Q[i] @ self.P[u].T
            )
            mean_err += err
    ...
    def init_weights(self):
        ...
        self.P = np.random.normal(
            self.mean, self.std, size=(self.n_users, self.n_factors)
        )
        self.Q = np.random.normal(
            self.mean, self.std, size=(self.n_items, self.n_factors)
        )
    ...
    def encode_df(self, X):
        self.le_users = OrdinalEncoder(
            handle_unknown="use_encoded_value", unknown_value=-1
        )
        self.le_items = OrdinalEncoder(
            handle_unknown="use_encoded_value", unknown_value=-1
        )
        self.X = pd.DataFrame(X)

        self.X["user"] = self.le_users.fit_transform(
            self.X["user"].values.reshape(-1, 1)
        )
        self.X["item"] = self.le_items.fit_transform(
            self.X["item"].values.reshape(-1, 1)
        )

    def transform_test(self, X_test):
        self.X_test = pd.DataFrame(X_test)
        self.X_test["item"] = self.le_items.transform(
            self.X_test["item"].values.reshape(-1, 1)
        )
        self.X_test["user"] = self.le_users.transform(
            self.X_test["user"].values.reshape(-1, 1)
        )
