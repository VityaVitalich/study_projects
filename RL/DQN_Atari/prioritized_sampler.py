...
class PrioritizedSampler:
    ...
    def sample_indices(self, batch_size):
        ...
        random_cum_prior = np.random.uniform(
            0, self.priorities.total_p, size=batch_size
        )
        indices = []
        for i in range(batch_size):
            cur_idx = self.priorities.get_leaf(random_cum_prior[i])
            indices.append(cur_idx)

        indices = np.array(indices)

        priorities = np.array(self.priorities[indices])

        probs = priorities / self.priorities.total_p
        weights = probs / self.priorities.capacity
    ...
    def update_priorities(self, indices, new_priorities):
        ...
        """
        new_priorities = (new_priorities**self.rp_alpha).clip(
            min=1e-5, max=self.clip_priorities
        )

        # update priorities
        for i in range(len(indices)):
            self.priorities.update(indices[i], new_priorities[i])
