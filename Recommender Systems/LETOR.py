...
class LambdaMart():
    ...
    def __init__(self, n_trees = 10, max_leaf_nodes = 10, lr = 0.01, max_depth = None, min_samples_split=2, max_rank=1):
    ...
    def compute_swaps_lambda(self, X):
        ...
        swaps['diff'] = np.abs((swaps['gain_x'] - swaps['gain_y']) * (swaps['dc_x'] - swaps['dc_y']))
        swaps['lambda'] = 0
        
        good_swap = swaps[swaps['relevance_x'] > swaps['relevance_y']]
        swaps.loc[swaps['relevance_x'] > swaps['relevance_y'], 'lambda'] = good_swap['diff']
        
        lambdas = swaps.groupby(['query_id', 'rank_sorted_x'])['lambda'].sum() - swaps.groupby(['query_id', 'rank_sorted_y'])['lambda'].sum()
...
def mean_nDCG(df, n, max_rank=1):
    ...
    avg_ndcg_ls = []
    for q in (querys):
        relevance_vector = df[(df['query_id'] == q)].sort_values(by='prev_res', ascending=False)['relevance'].values
